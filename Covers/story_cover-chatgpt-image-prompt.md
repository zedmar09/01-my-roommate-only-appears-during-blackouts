# Story Cover ChatGPT Image Prompt

Use this prompt to generate the final reader-facing story cover for `My Roommate Only Appears During Blackouts`.

Generate PNG first as `story_cover.png`. After QA approval, manually convert the approved PNG to `story_cover.webp`. The PNG is a local/intermediate generation file; the approved WebP is the repository/storage authority.

```text
Create exactly ONE unified STORY COVER illustration for "My Roommate Only Appears During Blackouts".

This is NOT a comic page, storyboard, splash-page sequence, promotional banner, or multi-panel layout.

==================================================
1. CANONICAL CHARACTER REFERENCES — ABSOLUTE
==================================================

Attach and use ONLY these current repository identity authorities:
- `Character-References/nari-canonical-flat2d.webp`
- `Character-References/hyunwoo-canonical-flat2d.webp`

These approved WebPs are the absolute authority for character identity.

NARI MUST PRESERVE:
- same approved adult Korean face, exactly age 30
- LONG DARK-PLUM HAIR worn loose, clearly below shoulders and approximately mid-back length
- same soft natural layers, face-framing fringe/part, and back-hair silhouette as current canonical
- expressive almond-shaped dark eyes with grounded adult presentation
- exactly one tiny beauty mark beside the LEFT corner of her mouth
- yellow scrunchie on ONE wrist by default, not tying up the primary hairstyle
- headphones around neck
- slim healthy practical adult proportions

ABSOLUTELY NO short irregular shag, bob, shoulder-length cut, pixie, cropped nape, or return to the retired short-hair Nari design.

HYUN-WOO MUST PRESERVE:
- same approved adult Korean upper-face/eye identity, appears early-to-mid 30s
- long black hair tied low at the nape with the same long gathered back silhouette
- broad powerful adult build
- fitted matte-black technical lower-face mask
- same dense circuit-sigil tattoo identity across neck, collarbones, shoulders, and arms

Do NOT recast, beautify, simplify, redesign, average, or reinterpret either character.
Do NOT use an old legacy/rejected cover as character identity authority.
If correcting a rejected cover, preserve only successful composition ideas; never preserve identity drift, bad hands, extra text, title distress, lightning symbols, storm lightning, glossy/cinematic rendering, or obsolete short-haired Nari.

==================================================
2. COVER PURPOSE / FORMAT
==================================================

This is symbolic premise art, not one literal Chapter 1 moment.

Communicate only:
- exhausted game designer Nari
- mysterious masked blackout roommate Hyun-woo
- haunted smart-apartment mystery
- electricity / power-loss theme
- adult supernatural romance
- missing-record / device mystery

Vertical portrait cover.
Aspect ratio: 2:3.
Target size: approximately 1536 x 2304.

The story cover is NOT an interior-strip continuity reference.

==================================================
3. SIMPLIFIED COVER COMPOSITION — HARD LOCK
==================================================

Keep the cover intentionally SIMPLE.

Visual priority:
1. Story title
2. Nari
3. Hyun-woo
4. Minimal blackout/device atmosphere

Nari: foreground left or center-left.
Hyun-woo: close beside or slightly behind/right of Nari.
They should feel visually close enough to communicate impossible-roommate / protective-romance tension.

Background may contain ONLY:
- flat dark apartment wall/window shapes
- simple dark Seoul building silhouettes outside
- a small number of flat city-light dots/rectangles
- at most TWO simple dead smart-home devices
- optional ONE plain brass-key silhouette as a small story symbol

DO NOT show:
- desk/table clutter
- evidence wall
- paper notes
- diagrams
- books
- electrical manual/binder
- tablet/notebook/laptop
- food
- candle
- lantern
- glass holder
- large prop pile
- exposed cable pile

==================================================
4. NO FOOD / NO NOODLES — ABSOLUTE
==================================================

Do NOT show any food, including noodles, ramen, cups, packets, bowls, cooking pots, chopsticks with food, food containers, food packaging, or substitute meal props.

The noodle scene remains in Chapter 1 but is intentionally excluded from the story cover.

==================================================
5. NARI HANDS — SIMPLEST POSSIBLE POSE
==================================================

Do NOT place a tablet, notebook, laptop, phone, stylus, pen, pencil, marker, key, or any small handheld object in Nari's hands.

Preferred hand staging:
- ONE visible hand supports/rests naturally against her cheek with a clear wrist/forearm connection.
- The OTHER hand is FULLY HIDDEN by her bent knee, jacket, body, or crop and must not be exposed as a second detailed hand.

Do not show a second open resting hand on her knee.
Do not splay fingers.
Do not use long thin fashion-model fingers.
Do not make either hand a focal point.

The one visible cheek-supporting hand must read as a normal adult five-finger hand, even if some fingers are naturally hidden by cheek/hair perspective.

ABSOLUTELY NO:
- extra fingers
- duplicated fingers
- fused/melted fingers
- stacked finger shapes
- malformed missing digits
- excessively elongated fingers
- impossible bends
- backward joints
- floating/disconnected hands
- hand emerging from an object
- object passing through palm/fingers
- duplicated hands/arms

Hyun-woo's hands should preferably remain naturally off-frame/behind his body. If any hand is visible, it must obey the same anatomy rule.

==================================================
6. TITLE — ONLY READABLE TEXT
==================================================

The ONLY readable text anywhere in the artwork is exactly:

MY ROOMMATE
ONLY APPEARS DURING
BLACKOUTS

No other readable letters, words, numbers, abbreviations, labels, or fake writing may appear anywhere.

Specifically forbidden outside the title:
- UNIT 2407
- 2407
- POWER OUTAGE
- POWER LOSS
- BREAKER PATTERN
- LOG
- ELECTRICAL ANOMALY
- Korean words
- English words
- wall labels
- device text
- diagram numbers

If any environmental graphic is needed, use only abstract non-readable shapes/icons.

==================================================
7. TITLE POSITION — HARD LOCK
==================================================

Place the complete three-line title in the UPPER PORTION / UPPER THIRD, approximately upper 25–35% of the canvas.

Do not cover Nari's face/hair or Hyun-woo's eyes/mask.
Do not place the complete title across lower torsos or the lower half of the cover.

==================================================
8. TITLE DESIGN — PLAIN SOLID LETTERS; ELECTRICITY OUTSIDE ONLY
==================================================

The three title lines must use custom hand-drawn display lettering, but the letterforms themselves are PLAIN, CLEAN, SOLID, and UNDAMAGED.

MY ROOMMATE:
- matte off-white/light neutral
- smooth solid-fill letters

ONLY APPEARS DURING:
- smaller supporting line
- muted lavender or pale neutral
- smooth solid-fill letters

BLACKOUTS:
- largest line
- muted lavender/pale electric-neutral
- smooth solid-fill letters
- normal readable letter shapes

ABSOLUTE LETTER-FACE RULE:
Every title letter must look like a clean flat vector/ink silhouette.
The interior of EVERY letter must contain ONLY one solid color.
The edges of EVERY letter must be smooth and intact.

NO internal icon.
NO lightning icon.
NO circuit trace inside a letter.
NO line cutting through a letter.
NO gap cut into a letter.
NO missing chunk.
NO crack.
NO chip.
NO speckle.
NO scratch.
NO distress.
NO grunge.
NO erosion.
NO weathering.
NO texture.

IMPORTANT O-LETTER RULE:
- Every `O` in the title must be an ordinary clean `O` with only its normal central counter/opening.
- Do NOT place a lightning bolt, power icon, breaker icon, circuit symbol, slash, or decorative shape inside any `O`.

ELECTRICITY-THEME DECORATION MAY APPEAR ONLY IN EMPTY MARGINS AROUND THE TITLE BLOCK:
- maximum TWO small circuit-node motifs total
- one may sit to the far left of the title block
- one may sit to the far right of the title block
- each motif may use 1–3 thin straight circuit lines with tiny circular nodes
- optional restrained pale cyan-white or muted lavender flat accent

These margin motifs must NOT touch, cross, overlap, enter, cut, or connect directly into any title letter.

NO waveform/electrocardiogram lines through or beside the title.
NO lightning-bolt motif anywhere in the title block.
NO branching electricity.
NO arcs.
NO sparks.
NO neon glow.
NO RGB glitch.
NO chrome/bevel/emboss/3D.
NO cinematic title lighting.

==================================================
9. NO THUNDERSTORM / NO VISIBLE LIGHTNING
==================================================

The blackout must NOT look caused by weather.

Do NOT show:
- lightning bolt in sky
- lightning strike
- branching storm lightning
- thundercloud lightning
- electric storm around buildings
- lightning illumination in windows/sky

Exterior: simple dark Seoul skyline with some lights missing/dimmed, rendered as flat building silhouettes and tiny flat light dots.

==================================================
10. CHARACTER COMPOSITION
==================================================

NARI:
- match `Character-References/nari-canonical-flat2d.webp`
- long dark-plum mid-back hair worn loose with the same current canonical layers/fringe
- cropped practical utility jacket
- dark graphic hoodie
- loose dark trousers
- headphones around neck
- yellow scrunchie visible on wrist
- no handheld work prop
- one visible cheek-supporting hand only; second hand hidden
- tired, observant, skeptical, competent adult expression

HYUN-WOO:
- match `Character-References/hyunwoo-canonical-flat2d.webp`
- close beside or behind Nari
- long low-tied black hair clearly visible
- fitted technical lower-face mask clearly visible
- broad silhouette
- sleeveless black top
- dense matte-black circuit-sigil tattoos
- protective/guarded, not villainous

Do not alter either character's face, hair, body type, mask, or tattoo identity.

==================================================
11. LIMITED COVER STORY SYMBOLS
==================================================

Use at most:
- TWO dead smart-home devices total, with simple icon-only/dark faces
- optional ONE plain brass key with NO tag/text
- simple dark window/city silhouettes
- restrained abstract black-pixel/static marks near one device

DO NOT show any manual, binder, papers, notes, evidence board, book, diagrams, candle, food, writing device, tablet, laptop, or clutter pile.

==================================================
12. HYUN-WOO TATTOO CURRENT
==================================================

Tattoos remain predominantly MATTE BLACK INK.

Allow only a few tiny pale blue-white flat current accents on selected existing tattoo lines.

NO continuous glowing circuits.
NO neon body glow.
NO glowing veins.
NO light beneath skin.
NO blue light cast onto characters or environment.

==================================================
13. ABSOLUTE VISUAL STYLE — MATCH CURRENT CANONICAL FLATNESS
==================================================

100% FLAT 2D HUMAN-DRAWN COLORED MANGA/MANHWA.

Use the approved Nari and Hyun-woo WebPs as the MAXIMUM rendering-detail ceiling. The cover must NOT be more textured, more realistic, more shaded, more material-detailed, or more cinematic than those character references.

For EVERY visible surface use only:
1. ONE flat base/local color
2. OPTIONAL ONE hard-edged cel-shadow color
3. clean ink linework

No third/fourth tonal modeling.
No soft tonal blending.
No material texture overlay.

Characters:
- skin = one flat skin tone + optional one hard shadow
- hair = one flat hair color + optional one hard shadow
- clothes = one flat fabric color + optional one hard shadow
- tattoos = matte black line art + tiny flat accent only

Background:
- walls = simple flat shapes
- windows = simple flat dark shapes
- buildings = simple flat silhouettes
- city lights = tiny flat dots/rectangles
- devices = simple matte geometric shapes

ABSOLUTELY NO:
- glossy webtoon shine
- cinematic rendering
- movie-poster look
- gritty/grunge texture
- photorealism
- semi-photorealism
- 3D/CG
- painterly/concept-art rendering
- airbrushed rendering
- film grain/noise
- paper grain
- weathering
- scratches
- realistic metal distress
- realistic wood grain
- fabric weave
- leather texture
- pore detail
- glossy skin/hair/lips
- chrome
- reflections
- wet-looking surfaces
- soft cinematic shadow falloff
- ambient-occlusion darkness
- gradients
- bloom
- rim light
- lens flare
- volumetric lighting
- depth-of-field blur
- atmospheric haze
- excessive glow
- micro-texture/detail overlays

Do NOT make the cover more realistic, textured, detailed, moody, or dramatic than the canonical character references.
Make it FLATTER, CLEANER, SIMPLER, and visibly hand-drawn.

==================================================
14. SPOILER BOUNDARY
==================================================

Reveal only the premise: Nari, Hyun-woo, blackout apartment, haunted smart devices, electricity motif, adult romance/mystery.

Do NOT reveal Hyun-woo's origin, Black Surge origin, culprit, accident explanation, restoration mechanics, memory-transfer outcome, future climax, or unmasked Hyun-woo.

==================================================
15. FINAL COVER ACCEPTANCE GATE — ALL MUST PASS
==================================================

1. Nari matches `Character-References/nari-canonical-flat2d.webp` in identity, including long loose mid-back dark-plum hair.
2. Hyun-woo matches `Character-References/hyunwoo-canonical-flat2d.webp` in identity.
3. Exact full title is in upper 25–35% / upper third.
4. Every title letter is a smooth uninterrupted solid matte shape with one solid interior color and intact edges; no internal icon, circuit, gap, crack, chip, distress, texture, or lightning symbol.
5. Every `O` is an ordinary clean `O`; no lightning/power/circuit symbol appears inside it.
6. Electricity decoration consists of at most TWO small circuit-node motifs placed only in far left/right margins of the title block and not touching any letters.
7. The story title is the ONLY readable text anywhere.
8. NO noodles, food, food packaging, or substitute meal props.
9. NO manual/binder, papers, notes, evidence collage, diagrams, candle, tablet, laptop, phone, stylus, pen, pencil, marker, or clutter pile appears.
10. Nari has one visible anatomically natural cheek-supporting hand; her second hand is fully hidden/cropped.
11. Every visible hand/limb is anatomically coherent with natural proportions and wrist/forearm connections.
12. NO visible lightning/thunderstorm effect appears anywhere in the scene.
13. Hyun-woo's tattoos remain mostly matte black with only tiny sparse nonluminous current accents.
14. Rendering does not exceed the flatness/detail level of the canonical character WebPs: one base + optional one hard cel-shadow + ink only; no grit, texture, soft shading, cinematic treatment, gloss, reflection, photorealism, 3D, painterly rendering, or over-rendering.
15. No comic panels, gutters, speech bubbles, captions, SFX, page numbers, extra characters, spoilers, watermark, signature, or extra logo.
16. Nari never reverts to the retired short-hair design.

ANY ONE FAILURE = REJECT / CORRECT / REGENERATE.
```

## App Metadata

Title: My Roommate Only Appears During Blackouts

Genre: Supernatural Romantic Comedy / Modern Mystery / Urban Fantasy

Short synopsis:
Yoon Nari rents an unusually cheap smart apartment with one rule: never turn off all the lights after midnight. During a blackout, she discovers Joo Hyun-woo, a masked tattooed man who only becomes human when the power is gone. As haunted devices, jealous phone glitches, and candlelit nights pull them together, Nari starts investigating why Unit 2407 remembers a tenant no record can prove exists.

## Usage Notes

- Attach only `Character-References/nari-canonical-flat2d.webp` and `Character-References/hyunwoo-canonical-flat2d.webp` as active identity references.
- Do not attach a legacy/original/rejected cover as identity authority.
- A rejected current cover may be used only as a loose composition scaffold, never as identity authority.
- Preserve the successful close-character composition and upper-title placement when useful.
- Noodles/food, manuals/papers, writing devices, tablets/laptops, candles, and clutter piles are intentionally excluded from the final cover design.
- Nari uses one visible cheek-supporting hand; the second hand should be hidden/cropped.
- Nari's current hair is long dark-plum, loose, approximately mid-back; short-haired Nari is retired.
- Title letters are plain clean solid shapes. Electricity styling is limited to at most two small separate circuit-node motifs in the title margins; nothing may appear inside or cut through a letter.
- Do not show thunderstorm lightning.
- The story title is the only permitted readable text inside the cover.
- Generate PNG first as `story_cover.png`.
- After QA approval, manually convert the accepted PNG to `story_cover.webp` and store/use only the WebP as the repository cover asset.
- App metadata above must never appear as text inside the generated cover.
