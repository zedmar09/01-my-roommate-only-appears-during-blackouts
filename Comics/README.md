...
- Narration and time-caption wording is locked by the page script, but the VISUAL DESIGN is dynamic and scene-adaptive. Do not force every narration caption to use the same white box, background color, border, shape, size, or placement. Choose a clean high-contrast flat-2D treatment that blends naturally with the actual panel while staying clearly readable and visually distinct from dialogue, SFX, device text, notebook text, and panel-order markers.
- Meaningful time jumps belong inside the first establishing story panel after the jump as a LARGE, noticeable narration caption unless an individual page explicitly requests another treatment. Do not invent separate blank time-card gutters or transition strips by default.
- Every interior story panel must carry exactly one small circled reading-order number, consecutive from `1` through the page's locked panel count. Place it near the upper-left inside the panel or at a quiet gutter edge. Keep it readable but subtle, and never cover narration, dialogue, SFX, device text, notebook text, faces, hands, recurring props, or clues. Literal production labels such as `PANEL 1` are never printed.
...
- Keep each prompt self-contained with `CHARACTER CONSISTENCY`, `SETTING AND PROP CONTINUITY`, `SPOILER BOUNDARY`, panel-order-marker instructions, `TEXT AND LETTERING RULES`, `STORY CLARITY`, and `AVOID` blocks.
