# Page 001 ChatGPT Image Prompt

Generate Chapter 1, Page 1 as a PNG. Save the approved result as `1.png`, then manually convert it to `1.webp`.

```text
Create exactly ONE 1024 x 1536 vertical colored flat-2D human-drawn manga/manhwa page for Chapter 1, Page 1 of "My Roommate Only Appears During Blackouts".

REQUIRED REFERENCE
Attach `../../../Character-References/nari-canonical-flat2d.png`.
That PNG is the absolute authority for Nari's face, age, proportions, short asymmetrical dark-plum hair, and beauty mark.

NARI CONTINUITY
- Korean woman, age 30.
- SAME approved canonical face and proportions.
- Hair stays SHORT, LOOSE, and UNTIED in every panel.
- Yellow scrunchie stays on ONE WRIST only; never use it as a hair tie.
- Headphones around neck.
- Dark graphic hoodie under cropped utility jacket.
- Loose black trousers and canvas sneakers.
- Pixel-ghost keychain, if visible, is attached to her backpack/bag only.

STYLE
100% flat 2D human-drawn colored manga/manhwa.
Use clean ink, matte cel colors, solid color blocks, and at most one simple hard-edged cel-shadow tone.
No glossy rendering, cinematic rendering, photorealism, 3D/CG, painterly rendering, reflections, gradients, bloom, rim light, gritty texture, or over-rendering.

UNIT 2407 MASTER LAYOUT
- living room / matte-black TV on LEFT
- pale-gray dining table near CENTER
- EXACTLY TWO dark-burgundy dining chairs total
- kitchen / matte cool-gray refrigerator on RIGHT
- hallway / bedroom doors BEHIND
- dark-panel corner windows on exterior wall with a believable Seoul city view
- lower kitchen cabinets muted eucalyptus green
- floor medium neutral gray
- speaker is a SMALL screenless matte-charcoal corded cylinder plugged into the counter outlet

ARRIVAL PROPS
Nari has:
- backpack
- stickered tablet
- ONE carryable moving box labeled exactly: KITCHEN / DO NOT DROP
- small corridor moving cart with exactly TWO additional boxes

The moving cart stays outside in the corridor through Panels 2–4.

PAGE STRUCTURE
Use exactly SIX clearly separated panels.
Render exactly ONE small circled panel-order number inside every panel: `1`, `2`, `3`, `4`, `5`, `6`, matching the top-to-bottom reading order below.
Place each number near the upper-left inside its own panel or at a quiet gutter-edge position. Keep it clearly readable but subtle.
Never place a panel number over dialogue, captions, SFX, faces, hands, the moving box, the smart lock, the speaker, or other story clues.
Do not add any other production labels, page-title footer, chapter-title footer, credits, author name, watermark, or extra labels.

TEXT FIDELITY
Use ONLY the exact captions, dialogue, and SFX listed below.
Do not paraphrase, rewrite, add, omit, or move any line.
The only other readable text allowed is the apartment-door number `2407` where naturally visible in corridor/door views and the box label `KITCHEN / DO NOT DROP`.
Do NOT place `2407` or any other unit/building number on the exterior high-rise facade in Panel 1.

NARRATION CAPTION DESIGN
Narration/caption design is scene-adaptive, not fixed to one background style.
Blend each narration caption naturally into the actual panel while keeping it clearly readable and visually distinct from dialogue, SFX, device text, and the small circled panel-order number.
Depending on the local scene, use an appropriate flat 2D caption treatment such as a clean solid box, a restrained dark/light box, or another simple high-contrast caption shape that fits the composition.
Do NOT force every narration caption to use the same white background, color, shape, size, or placement.
Do NOT render narration as unreadable bare text over a busy background; maintain enough contrast and breathing room for easy reading.
Keep the caption treatment consistent with the mood and visual balance of its own panel without changing the exact caption wording.

PANEL 1
Wide exterior of the modern residential high-rise at night with a believable Seoul city setting. The building should look desirable, ordinary, and well maintained rather than ominous.
CAPTION: UNIT 2407 WAS TOO CHEAP.
No exterior `2407` sign or number on the building facade.
No other text.

PANEL 2
Corridor. Nari is still OUTSIDE the apartment, balancing the same moving box while tapping the smart lock. The small moving cart with exactly TWO boxes is clearly behind her in the corridor.
SFX: BEEP
CAPTION: IN SEOUL, CHEAP RENT ALWAYS HIDES A CATCH.
No other text.

PANEL 3
Nari pushes the apartment door open and visibly crosses the threshold carrying the SAME box. The reader must clearly see first entry from corridor into apartment.
SFX: CLICK
No caption. No dialogue. No other text.

PANEL 4
Nari has just set the SAME box down immediately inside. The box is visible on the floor. The front door remains open/visible, and the corridor cart with its TWO boxes remains visible through/near the doorway.
Use this wide panel to establish the fixed apartment layout: TV left, dining table center with EXACTLY TWO burgundy chairs TOTAL, kitchen/refrigerator right, hallway behind, corner windows on the exterior wall with a believable city view. Do not draw a third or fourth dining chair.
SFX: THUD
CAPTION: TWO BEDROOMS. CORNER WINDOWS. CITY VIEW.
CAPTION: ALL FOR LESS THAN MY OLD PLACE.
No other text.

PANEL 5
Nari scans the apartment and notices the SMALL plugged-in counter speaker. Its cord clearly reaches the outlet. Her expression is observant and mildly wary, not frightened.
CAPTION: SMART LOCK. SMART LIGHTS. SMART CLIMATE.
CAPTION: SMART EVERYTHING.
No dialogue. No other text.

PANEL 6
Close-up or medium close-up of the correctly sized screenless corded speaker, with Nari behind/beside it in dry profile.
SFX: BLIP
RECORDED SPEAKER VOICE, bubble tail points to the SPEAKER: WELCOME HOME, YOON NARI.
NARI: GREAT. IT TALKS.
No other text.

FINAL CHECK BEFORE GENERATING
- Exactly 6 panels in the order above.
- Exactly one small circled panel-order number per panel: `1` through `6`, with no missing, duplicated, or out-of-order numbers.
- Narration caption design adapts to each panel's scene and remains high-contrast/readable; do not force one fixed white-box style across all captions.
- Exact text only; no invented biography, tenant rules, narration, footer, or extra dialogue.
- Nari matches the canonical PNG and keeps loose untied short hair in every panel.
- Scrunchie stays on wrist.
- Panel 1 has NO exterior `2407` building number/sign.
- Panel 4 has EXACTLY TWO burgundy dining chairs total, never three or four.
- Panel 3 visibly shows threshold crossing.
- Panel 4 visibly shows box down + open door + corridor cart continuity.
- Use a believable high-floor Seoul CITY VIEW; do not force a river view or landmark-specific geography.
- Speaker stays small, screenless, corded, and plugged in.
- No Hyun-woo, blackout, hostile signal, supernatural manifestation, landlord warning, or future-story clue yet.
```

## Locked Reader-Facing Text

| PanelText | Text |
| --- | --- |
| 1 | `UNIT 2407 WAS TOO CHEAP.` |
| 2 | `IN SEOUL, CHEAP RENT ALWAYS HIDES A CATCH.` |
| 2 SFX | `BEEP` |
| 3 SFX | `CLICK` |
| 4 | `TWO BEDROOMS. CORNER WINDOWS. CITY VIEW.` |
| 4 | `ALL FOR LESS THAN MY OLD PLACE.` |
| 4 SFX | `THUD` |
| 5 | `SMART LOCK. SMART LIGHTS. SMART CLIMATE.` |
| 5 | `SMART EVERYTHING.` |
| 6 SFX | `BLIP` |
| 6 Speaker | `WELCOME HOME, YOON NARI.` |
| 6 Nari | `GREAT. IT TALKS.` |

## Required Panel Order Markers

Render one small circled number inside each panel in reading order: `1`, `2`, `3`, `4`, `5`, `6`.

## Production Thread Use

For a fresh generation, attach only:
1. `Character-References/nari-canonical-flat2d.png`
2. this current `page-001-chatgpt-image-prompt.md`

For a targeted correction of an otherwise correct Page 1, the current near-pass Page 1 image may be used as the edit target together with this `.md`; preserve all correct panels/text/characters and change only the explicitly identified defects.

Never use an older rejected Page 1, the legacy cover, or another page as continuity authority.
