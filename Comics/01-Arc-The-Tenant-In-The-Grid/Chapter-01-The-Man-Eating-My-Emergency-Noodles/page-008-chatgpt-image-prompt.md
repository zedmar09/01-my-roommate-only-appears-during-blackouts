# Page 008 ChatGPT Image Prompt

Copy the prompt below into ChatGPT to generate Chapter 1, Page 8 as PNG. Save it as `8.png`, then manually convert it to `8.webp`.

```text
Create a colored flat 2D human-drawn modern supernatural romance manga/manhwa comic page for Chapter 1, Page 8 of "My Roommate Only Appears During Blackouts".

IMPORTANT REFERENCES
Use `../../../characters.md` and `../../style-guide.md`; preserve approved Page 7 for page-to-page continuity.

CANONICAL CHARACTER REFERENCE LOCK
Attach the APPROVED flat-2D canonical Nari reference created from `../../../Character-References/nari-canonical-flat2d-chatgpt-image-prompt.md` and the APPROVED flat-2D canonical Seungjae reference created from `../../../Character-References/seungjae-canonical-flat2d-chatgpt-image-prompt.md`.
Those canonical references are the absolute authority for each character's face, hair, adult age, persistent identifiers, and stable body proportions. Approved Page 7 controls ongoing rendering continuity only and cannot override canonical identity. This page prompt controls current after-work wardrobe, poses, expressions, devices, dialogue, and scene.
Do not attach the legacy/original cover, rejected/drifted covers, or unapproved character generations.

STYLE TARGET - COLORED FLAT 2D MODERN MANGA/MANHWA
HARD STYLE BAN: no semi-photorealism, glossy rendering, cinematic rendering, 3D/CG, painterly treatment, reflections, or over-rendering.
Use 100% flat 2D human-drawn manga/manhwa artwork with clean ink, matte cel colors, solid blocks, adult proportions, and one hard-edged shadow. Apply to people, umbrella, lobby, glass doors, elevator, phone, earbuds, watch, key, tablet, and backgrounds. No glossy/cinematic/photoreal/3D/CG/painterly/airbrushed/reflective/bloom/rim-light/flare/volumetric/gradient/blur/glare/wet/shiny/over-rendered treatment; lobby glass is a flat transparent boundary without reflections.

ENVIRONMENT AND APPLIANCE MASTER LOCK
Ground-floor lobby has matte stone/color blocks, glass exit doors without mirror glare, elevator doors and call button at residential scale.

LIGHTING AND EXPOSURE LOCK
One medium-bright neutral powered baseline. Device failure uses tiny contained static marks only.

POWER AND SUPERNATURAL RULE LOCK
Nonphysical Hyun-woo causes petty clean-signature battery interference. Seungjae's phone, earbuds, smartwatch, and power bank fail; his electronic car key visibly still works. Do not create transport danger. Hyun-woo's tablet text is clean stable cyan-white without hostile black pixels.

CHARACTER CONSISTENCY
Nari is the SAME approved canonical 30-year-old Korean woman in after-work jacket, trousers, sneakers, headphones, shoulder bag with stickered tablet. Seungjae is the SAME approved canonical Korean man, 33, with neat brown hair, office-casual clothes, closed umbrella, foldable phone, earbuds, smartwatch, and practiced smile. Do not reinterpret either face from text.

SETTING AND PROP CONTINUITY
They enter lobby together from rain. Seungjae's separate devices fail in sequence. Car-key indicator flashes normally. Nari's tablet is inside her bag. Final panel must show elevator doors open, Nari stepping inside, Seungjae turning toward exit, then doors starting to close.

OBJECT SCALE AND SPATIAL LOGIC
All electronics stay correctly sized and attached/held naturally. Do not combine devices into one object.

CONTINUITY
Two days after preparation; Hyun-woo's jealousy remains funny, not dangerous.

TIME AND SCENE CONTINUITY
Exterior/lobby arrival to elevator departure in one continuous scene. No flashback.

TIME TRANSITION CARD
Before Panel 1, add an unnumbered quiet full-width gutter containing only: TWO DAYS LATER, AFTER WORK

SPOILER BOUNDARY
No physical Hyun-woo, hostile entity, blackout, or future lore.

PAGE FORMAT AND PACING
One vertical page with 6 panels, light romantic-comedy timing.

PANEL ORDER NUMBERS
Add small circled numbers 1-6, not on time card or over text/faces/hands/devices.

PAGE LAYOUT AND SCRIPT
PANEL 1 - Nari and Seungjae enter lobby under a closed umbrella; he smiles toward her.
SEUNGJAE: DINNER SOMETIME? WHEN WE ARE NOT SHIPPING A CRISIS.

PANEL 2 - His foldable phone, earbuds, smartwatch, and power bank fail in a readable sequence.
SFX: ZZT. ZZT. ZZT.
SEUNGJAE: WHY DID EVERYTHING JUST DIE?

PANEL 3 - He presses car key; its small indicator works normally.
SEUNGJAE: AT LEAST I CAN GET HOME.

PANEL 4 - Inside Nari's bag, tablet wakes with clean cyan-white text.
TABLET: THAT GUY: LOW POWER MODE

PANEL 5 - Nari whispers toward tablet; it replies beneath her line.
NARI: ARE YOU TWELVE?
TABLET: OLDER

PANEL 6 - Elevator doors are open. Nari visibly steps inside while Seungjae turns toward exit; doors begin closing between them.
SEUNGJAE: I WILL TEXT YOU WHEN MY PHONE REVIVES.
NARI: GET HOME FIRST.

SFX AND TEXT BLENDING
Keep battery-failure SFX small and comic, never menacing.

TEXT AND LETTERING RULES
Use only exact uppercase text. Tablet message is clean cyan-white. No hostile corruption or labels on glass.

STORY CLARITY
The reader must understand Hyun-woo's interference is petty but not transport-dangerous and the lobby scene physically closes before the next time jump.

AVOID
Avoid Nari or Seungjae identity drift, generic replacement faces, dead car key, trapped exits, any Hyun-woo-caused elevator malfunction, Nari only standing at call button, reflected characters in glass, hostile signature, physical Hyun-woo, glossy/cinematic effects, extra text, and flashback.
```
