...
- `chapter-02-generation-checklist.md`
- `characters.md`
...
Only these three pages may skip elapsed time/location:

| Page | Exact caption | Why the jump is legitimate |
|---:|---|---|
| 3 | `THE NEXT MORNING — UNIT 2407` | overnight sleep/hygiene and ordinary meal-only cleanup before Mrs. Na arrives |
| 8 | `LATER THAT DAY — NTH PIXEL STUDIO` | Mrs. Na leaves normally; Nari commutes to work and brings normal belongings |
| 10 | `LATER THAT NIGHT — UNIT 2407 — AFTER MIDNIGHT` | Nari commutes home, returns to established home outfit, and positions phone/notebook before test |
...
| Panel counts | PASS | `6,6,6,7,6,7,8,6,6,8,6,6,7,6,6,7,7,6`. |
| Panel markers | PASS | One small circled consecutive marker per story panel. |
...
| P11 route | PASS | table → living switch → bedroom switch → dining switch → table. |
| P12 state | PASS | Nari/notebook remain table; hall lamp flickers itself. |
| P13 state | PASS | hall lamp self-off; supply stays on; phone counter; notebook table. |
| P14 route | PASS | dining area → hallway switch physically; phone/notebook stay behind. |
| P14 visibility | PASS | restrained city/window ambient + powered indicators; no impossible pitch-black/cinematic illumination. |
| P15 dialogue | PASS | simple darkness vs power-loss distinction. |
| P16 dialogue | PASS | device access explained simply; no magical naming rule. |
| P17 numbering | PASS | 001 → 002 → 003 → 004. |
| P17 complete rule | PASS | after-midnight qualifier included. |
| P17 lighting | PASS | dining light restored before detailed writing. |
| P18 ending | PASS | both lights steady; panel closed; Nari seated; `TIK` source exact. |
| Phone path | PASS | home counter → natural narrated work travel → re-established counter P10 → fixed through P18. |
...
Reject any generated Chapter 2 page showing:
- glossy webtoon shine / shiny plastic surfaces
- cinematic film lighting/color grading
- photoreal/semi-photoreal faces/environments
- 3D/CG
- painterly/airbrushed
- mirror-like reflections on TV/refrigerator/windows/floors/counters
- glass glare streaks
- bloom/rim light/halos/lens flare/volumetric rays
- soft gradients
- wet-looking floors/counters
- excessive electrical glow lighting room
- depth-of-field blur
- over-rendered reflective hair/skin/appliances

Require clean human-drawn ink, matte cel colors, solid blocks, adult proportions, readable backgrounds, nonreflective appliances, restrained hard-edged cel shading.

## Movement / Event Rejection Gate — ABSOLUTE
...
- changes Nari back into work clothes after P10
- makes Nari write P17 notes before restoring dining light
- teleports Nari to P14 switch
- creates real Ch2 blackout
- manifests physical Hyun-woo
- opens/touches south-wall panel
- changes appliance/object location merely for composition
