...
Copy the prompt below into ChatGPT to generate Chapter 2, Page 13 as PNG. Save as `13.png`, then convert to `13.webp`.

```text
Create exactly ONE vertical colored flat-2D human-drawn manga/manhwa page for Chapter 2, Page 13 of "My Roommate Only Appears During Blackouts".

IMAGE-ONLY EXECUTION LOCK
FINAL prompt. Execute literally; image only; no rewriting/reblocking/invention.

READER-FACING TEXT LOCK — ABSOLUTE
All SFX, device text, and dialogue is exact/source/panel locked.

IMPORTANT REFERENCES
Use `../../../characters.md`, `../../style-guide.md`, approved Page 12 as mandatory lighting/power/position continuity.

CANONICAL CHARACTER REFERENCE LOCK
Attach Nari canonical PNG. Hyun-woo remains voice-only and physically absent; do not draw him.

STYLE TARGET - COLORED FLAT 2D MODERN MANGA/MANHWA
Use 100% flat 2D human-drawn modern manhwa with clean ink, matte cel colors, solid blocks, adult proportions, and restrained hard-edged shading. No glossy/cinematic/photoreal/3D/painterly rendering, mirror reflections, glare, bloom, rim light, gradients, wet/shiny surfaces, or over-rendered reflective effects.

ENVIRONMENT AND APPLIANCE MASTER LOCK
Same Unit 2407. All ordinary room lights except hallway lamp are already off. Refrigerator and smart lock remain supplied/powered. Hallway lamp begins on and switches itself off in Panel 1. Refrigerator keeps exact body/location.

LIGHTING AND EXPOSURE LOCK
After hallway lamp switches off, the apartment becomes visually dark but MUST still read as supplied/powered: refrigerator hum/display capability, smart-lock indicator, and powered appliance status remain. This is NOT a blackout and must not use Chapter 1 blackout manifestation language. Keep the scene readable with matte ambient city/window light and existing powered indicators only; no cinematic glow or reflective highlights.
...
SPOILER BOUNDARY
Do not name/explain hostile entity.

PAGE FORMAT AND PACING
Exactly 7 panels.

PANEL ORDER MARKERS
One small circled marker `1`–`7` per panel.
...
FRIDGE / HOSTILE DEVICE TEXT: VISUAL ACCESS: OPEN
...
HYUN-WOO: TURN THE HALL LIGHT BACK ON.

PANEL 7 — Same urgent speaker source.
HYUN-WOO: NOW.

SFX AND TEXT BLENDING
`CLICK` belongs only to hallway lamp switching itself off. Hostile device text is stark broken/doubled white with crawling black pixels.
...
|---|---|---|
| 1 SFX | Hallway lamp switching itself off | `CLICK` |
| 3 Fridge | Hostile device text | `VISUAL ACCESS: OPEN` |
| 4 Fridge | Hostile device text | `YOON NARI` |
| 6 Hyun-woo | Voice through unplugged speaker | `TURN THE HALL LIGHT BACK ON.` |
| 7 Hyun-woo | Voice through unplugged speaker | `NOW.` |

## Required Panel Order Markers
`1` through `7`.

## Production Thread Use
Attach Nari canonical PNG, this prompt, approved Page 12. Hyun-woo stays voice-only.