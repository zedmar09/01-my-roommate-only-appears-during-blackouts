# Nari Apartment — Manga Environment Canon

## Status

**TEXT APPROVED — PNG VISUAL CANDIDATES PENDING / FINAL WEBPS PENDING.**

This is the semantic spatial authority for Nari's recurring home.

Generation/repository workflow:

- first master-atlas candidate: `nari-apartment-master-atlas.png`
- optional separate floor-plan candidate: `nari-apartment-floor-plan.png`
- after visual approval, the user manually converts each exact accepted PNG to the matching final WebP authority:
  - `manga/02-references/environments/nari-apartment/nari-apartment-master-atlas.webp`
  - `manga/02-references/environments/nari-apartment/nari-apartment-floor-plan.webp`
- only approved converted WebPs are repository canon

## Core Identity

- modest modern Seoul one-bedroom apartment
- approximately **40–45 m²**
- ordinary working-adult rental
- believable and comfortable but not luxurious
- not deliberately creepy, decayed, haunted-looking, hotel-like, or futuristic
- not a cramped one-room studio

The apartment must feel like a normal home first. Supernatural tension comes from changes inside a familiar stable space, not from inherently sinister architecture.

## Spatial Organization

### Entry

- front door opens into a short practical entry hall
- entry has normal shoe/storage logic
- entry orientation allows a useful sightline toward the living/dining zone
- front-door interior/exterior geometry must remain consistent across pages

### Bathroom

- compact bathroom positioned near the entry side of the unit
- door placement must not interfere with the main living sightline

### Kitchen

- compact modern kitchen adjoining the dining zone
- practical working-adult scale
- no luxury island kitchen
- kitchen relationship to dining table must be clear from multiple camera angles

### Dining Zone

- small rectangular dining table
- exactly **two matching dining chairs**
- table sits between/along the transition from kitchen to living area rather than isolated in a separate formal dining room

#### Chair A — Nari's Habitual Seat

- ordinary matching chair used by Nari
- position should stay stable enough to establish a routine

#### Chair B — Continuity Chair

This is the critical Chapter 001 physical marker.

**Known/reference state:**

- fully tucked at its designated side of the table
- chair back parallel to the table edge
- chair legs align against clear table/floor geometry so a later movement is visually obvious
- the designated side must never silently switch between pages

Chapter 001 eventually shows Chair B displaced after Nari deliberately restored it. No separate object canonical is needed; the environment reference controls its exact design, count, and reference position.

### Living Area

- main living area occupies the exterior/window side of the apartment
- normal sofa/seating/storage may be present but must remain modest and reproducible
- include a small home work/activity zone for Nari without making the room look like a professional office
- the living area must support quiet wide shots that clearly show whether another person is present or absent

### Bedroom / Private Route

- one separate bedroom/private room
- route from living/dining area to bedroom must be visually clear
- Chapter 001 ending depends on Nari leaving the living/dining space and becoming unaware of the chair, so this route must support clean continuity

### Windows / Exterior Side

- main window or small balcony-side opening on the living-room exterior wall
- keep exterior opening positions consistent
- do not redesign window size/placement depending on camera angle

## Immediate Building Common Route

The same reusable environment package should cover the minimum exterior common-area continuity needed for Chapter 001:

- Nari's front door exterior
- short corridor immediately outside her unit
- nearby elevator/common circulation point used for the morning neighbor complaint

Do not generate a whole apartment tower interior. Only the recurring immediate route is needed.

## Neighbor Relationship — Locked

The complaining neighbor lives **directly below Nari's unit**.

This is spatial canon for the Chapter 001 noise complaint:

- footsteps / human-weight floor movement are heard through the ceiling
- dining-chair scraping transmits downward
- no adjacent/shared-wall explanation is required

Do not depict the neighbor as adjacent unless canon is deliberately revised later.

## Chapter 001 Visual Continuity Priorities

1. front door appears secure/ordinary
2. apartment geometry remains stable during Nari's inspection
3. nothing obviously burglarized
4. dining table + two chairs stay identical throughout
5. Chair B known position is established clearly
6. Chair B remains in that position whenever visible through the ordinary-night sequence
7. final reader-only change must be unambiguous because the environment did not drift

## Atmosphere

- normal, lived-in, recently occupied by Nari for roughly two weeks
- modest personal organization and believable small everyday clutter allowed
- do not over-decorate with plot clues
- no horror props, occult objects, supernatural symbols, suspicious stains, or ominous architecture

## Manga Rendering

Use the approved series manga style reference WebP once available.

The environment should support:

- detailed establishing shots
- clean medium interior views
- simplified background use during dialogue/reaction
- quiet empty-room shots
- strong black-and-white readability at night without cinematic color-light logic

## Automatic Reject

Reject a generated PNG environment candidate if:

- room geometry changes between views
- it reads as luxury penthouse/hotel or horror ruin
- chair count/design changes
- Chair B designated side is inconsistent
- front door/window/bedroom/kitchen relationships contradict the floor plan
- common corridor/elevator relation is impossible or random
- the neighbor is encoded as adjacent instead of directly below
- rendering is glossy, color, photoreal, CGI, 3D, painterly, or cinematic concept art

Only after a PNG passes this gate should it be manually converted to its final WebP authority.
