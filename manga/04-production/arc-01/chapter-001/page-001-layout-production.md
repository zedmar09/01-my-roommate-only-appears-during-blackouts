# Page 001 Layout Reference Production

## 1. Identity / Status

- series: `My Roommate Only Appears During Blackouts`
- arc: `Arc 01 — title pending`
- chapter: `Chapter 001`
- page: `001`
- purpose: **composition / staging reference only**
- status: **READY FOR SEPARATE LAYOUT-REFERENCE GENERATION**
- review candidate: `page-001-layout-reference.png`
- approved composition authority after manual conversion: `page-001-layout-reference.webp`

This file generates the Page 001 **layout reference only**.

It does not generate the final Page 001 manga page.

---

## 2. Exact Attachments

When generating the layout reference, attach exactly:

1. `manga/04-production/arc-01/chapter-001/page-001-layout-production.md`
2. `manga/02-references/approved-webp/nari-canonical.webp`
3. `manga/02-references/approved-webp/nari-workplace-master-atlas.webp`

Do **not** attach:

- `series-manga-style-reference-a.webp`
- `series-manga-style-reference-b.webp`

They are not required for this story-page layout reference.

Attachment roles:

- this MD controls composition/story beats for the layout reference
- Nari canonical controls Nari identity
- workplace atlas controls office geometry

---

## 3. Layout Reference Quality

The target is **not a rough sketch** and not empty panel boxes.

Generate a clean, content-filled **2D manga layout draft/reference**.

It should already look like manga drawing:

- clean ink-like black line art
- clear panel borders
- readable faces and expressions
- readable character poses/gestures
- coherent office perspective
- major desks/books/proofs/props visible
- readable speech-balloon placement
- right-to-left manga flow
- black-and-white only

It may remain simpler than final art:

- simplified background micro-detail
- lighter/minimal screentone
- simplified texture
- less final rendering density

Do not generate:

- blank panel templates
- wireframes
- stick figures
- scribbly construction sketches
- cinematic lighting
- glossy/painterly rendering
- 3D/CG

The layout reference must be detailed enough that final page generation does not invent new camera angles, poses, blocking, major props or balloon locations.

---

## 4. Canvas

- portrait
- **1024 × 1536 px**
- **2:3**
- right-to-left manga reading
- conventional page margins/gutters
- black-and-white only

---

## 5. Story Purpose

Page 001 opens on ordinary adult work life.

Nari is visibly still at her publishing-company workplace after midnight. The page establishes that she is physically away from home during the later apartment-noise contradiction.

Tone:

- normal deadline fatigue
- grounded adult routine
- dry work humor
- not horror
- not cinematic

---

## 6. Four-Panel Composition Target

Use **4 panels** with natural manga page rhythm.

### Panel 1 — Office Establishing

- dominant upper panel
- Nari at her recurring desk
- publishing/editorial office clearly readable
- manuscripts/proofs/books visibly important
- computers secondary
- some desks empty
- late-night context visible without cinematic lighting

### Panel 2 — Coworker Check-In

- smaller dialogue panel
- unnamed coworker near Nari's desk, preparing to leave
- ordinary body language
- balloon placed for right-to-left flow

Dialogue:

`You're still here?`

### Panel 3 — Nari Working

- larger character/work panel
- Nari remains focused on printed proofs
- readable hand action
- tired, composed expression
- balloon placement established

Dialogue:

`Just one more pass.`

### Panel 4 — Time / Location Proof

- quiet lower panel with breathing room
- Nari still working
- office mostly quiet/empty
- ordinary visual cue indicates after midnight
- analog wall clock may indicate approximately 12:25–12:30
- no dialogue

---

## 7. Composition Authority Output

After the PNG is visually approved, manually convert the exact accepted image to:

`manga/04-production/arc-01/chapter-001/page-001-layout-reference.webp`

Once committed, that WebP controls the final Page 001:

- panel boundaries
- relative panel sizes
- camera/framing
- character blocking
- pose direction
- major office/prop placement
- negative space
- speech-balloon positions

The final page may refine drawing/detail but must not redesign the approved composition without explicit user approval.

---

## 8. Automatic Reject

Reject the layout reference if:

- it is only panel boxes or a wireframe
- it is visibly rough/scribbly construction art
- story content is missing inside panels
- Nari is unrecognizable
- office does not read as publishing/editorial
- camera/framing is unclear
- poses/blocking are unclear
- balloons are missing in dialogue panels
- reading order is unclear
- it becomes cinematic/poster-like
- it becomes glossy/painterly/photoreal/3D
- output is not 1024×1536 portrait

---

## 9. Generation Instruction

Create exactly **ONE 1024×1536 portrait black-and-white 2D MANGA LAYOUT REFERENCE** for Chapter 001 Page 001.

This is a clean content-filled manga composition reference, not an empty layout template and not a rough scribble sketch. Draw all four story panels with readable characters, poses, facial acting, office staging, major props and speech-balloon placement.

Use `nari-canonical.webp` only for Nari identity and `nari-workplace-master-atlas.webp` only for workplace geometry.

Do not attach or imitate Style A/B. Do not use cinematic lighting, painterly shading, glossy rendering, photorealism or 3D.

Keep the visual language clearly 2D manga. The result can be simpler than final art but must be clean and composition-complete.

Output conceptually as `page-001-layout-reference.png` for review.

Final layout-reference approval status: **PENDING GENERATION BY USER**