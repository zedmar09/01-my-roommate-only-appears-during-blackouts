# Chapter 001 Pages Generation Guide

## Status

**APPROVED PRODUCTION GUIDE — READY TO COMPILE PAGE PRODUCTION FILES.**

This file controls the generation workflow for all 36 pages of Chapter 001. It does not replace the story files under `manga/03-story/`; it translates their approved story rhythm into a repeatable page-generation process.

## Chapter Function

Chapter 001 begins Arc 01 as an ordinary apartment-security mystery.

Nari is provably away at her publishing-company workplace while the resident directly below her apartment hears footsteps and a dining-chair scrape from Nari's supposedly empty unit. The next morning the neighbor complains. Nari checks ordinary explanations and access history, finds no normal entry, later notices one dining chair slightly out of place, deliberately restores it to a known position, then continues an ordinary evening. After Nari leaves the living/dining area and is unaware of the room, the reader alone sees/hears that the chair moved again.

Chapter question:

**Who — or what — is using Nari's apartment when nobody is supposed to be inside?**

## Absolute Reveal Ceiling

Chapter 001 must never show or identify:

- Joo Hyun-woo
- a male silhouette
- a human hand/foot/body fragment belonging to the unseen presence
- a male voice from the apartment
- Hyun-woo's name
- a roommate claim
- the blackout manifestation rule
- a reader-facing explanation connecting electricity to the chair movement
- record/memory mythology
- wider city mechanism
- antagonist/origin/restoration material

The ending may show only the changed chair / restrained chair-scrape evidence.

## Supporting-Cast Decision For Chapter 001

**No reusable Hyejin visual reference is required for Chapter 001.**

Chapter 001 may use unnamed/background publishing-office coworkers for ordinary social proof. Any optional phone/friend voice during the domestic-night sequence remains offscreen and must not require a visible canonical character.

The downstairs neighbor is chapter-local. Establish one ordinary adult appearance on Page 006 and preserve it through Pages 007–010 using adjacent-page continuity. Do not create a full neighbor canonical for this chapter.

## Page Canvas Lock

All Chapter 001 first-generation pages use:

- orientation: portrait
- PNG review candidate: **1024 × 1536 px**
- aspect ratio: **2:3**
- reading direction: **right-to-left manga reading**
- black-and-white manga only
- consistent external page canvas across all 36 pages
- panel count/geometry changes according to story rhythm
- no vertical-webtoon dead space

After a PNG passes visual review, the user manually converts that exact accepted PNG to the matching `.webp` final authority.

Example:

`page-001.png` → approve → user converts → `page-001.webp`

## Approved Reusable WebP Pool

All reusable image authorities are centralized under:

`manga/02-references/approved-webp/`

### Style — attach whenever generating a Chapter 001 page

1. `manga/02-references/approved-webp/series-manga-style-reference-a.webp`
2. `manga/02-references/approved-webp/series-manga-style-reference-b.webp`

Style A + Style B control rendering language only. They never control story-character identity or apartment/workplace geometry.

### Character

3. `manga/02-references/approved-webp/nari-canonical.webp`

Attach whenever Nari is visible. This is Nari's identity authority.

### Environments

4. `manga/02-references/approved-webp/nari-apartment-master-atlas.webp`
5. `manga/02-references/approved-webp/nari-workplace-master-atlas.webp`

The apartment master atlas already contains sufficient spatial/floor-plan information for Chapter 001. **No separate floor-plan image is required.**

## Attachment Principle

Do not attach every available WebP to every page.

Use only the references that control something visible on the page:

- Nari visible → Style A + Style B + Nari canonical
- workplace visible → add workplace atlas
- apartment/corridor/elevator visible → add apartment atlas
- empty apartment → Style A + Style B + apartment atlas; do not attach Nari unless she is actually visible
- same scene continues → attach immediately previous approved page WebP when it materially helps pose, neighbor identity, environment angle, chair state, or visual seam continuity

More attachments are not automatically better. Irrelevant references can cause identity/environment drift.

## Style Priority

Use the approved paired references as complementary visual authorities:

- Style A: strong finished ink, anatomy, hands, facial construction, hatching, solid-black control
- Style B: adult-female rendering, ordinary life, domestic interiors, quiet suspense, restrained expression, environment density/simplification

Nari's canonical overrides any generic human identity shown in Style A/B.

Environment atlases override any generic room/office shown in Style A/B.

## Chapter-Wide Visual Rhythm

Chapter 001 is not a horror page sequence from beginning to end.

Required tonal progression:

1. ordinary adult work life
2. mundane neighbor irritation
3. rational security contradiction
4. practical inspection
5. weak chair clue
6. deliberate known-state setup
7. comfortable domestic decompression
8. quiet empty-room suspense
9. restrained reader-only physical proof

Use darker blacks/hatching only when the scene earns them. Ordinary pages should retain substantial white space and normal manga readability.

## Sound / SFX Language

Keep Chapter 001 SFX restrained and physical.

Primary sounds:

- `THUMP` — human-weight floor movement / footstep transmitted downward
- `SKRRR` — dining chair/furniture scrape; the signature sound of Chapter 001
- `CLICK` — lock/light/small ordinary mechanism when needed
- `TAP` — phone/access-history interaction when useful
- `BEEP` — ordinary smart-lock/device acknowledgement when useful
- `BZZT` — optional phone vibration
- `SHHK` — optional curtain/fabric movement

Do not use giant supernatural impact lettering, electrical crackle, horror roar, or explanatory blackout SFX.

## Chair B Continuity Lock

The second dining chair is the physical continuity anchor.

Before Page 024:

- it may appear slightly displaced when Nari notices it
- Nari cannot prove who moved it

Pages 024–025:

- Nari deliberately pushes Chair B into the exact known/reference position encoded by the apartment atlas
- chair fully tucked at designated side
- chair back parallel to table edge
- table/floor geometry clearly proves alignment

Pages 025–034:

- Chair B must remain in that exact known position whenever visible
- no environment-generation drift may make it appear moved accidentally

Page 035:

- `SKRRR` occurs with source completely unseen

Page 036:

- Chair B is visibly displaced from the known position
- Nari does not witness this

## Page-by-Page Generation Matrix

Legend:

- `A` = Style A
- `B` = Style B
- `N` = Nari canonical
- `APT` = Nari apartment master atlas
- `WORK` = Nari workplace master atlas
- `PREV` = immediately previous approved page WebP

| Page | Panels Guide | Core Beat | Required WebPs | Local Continuity |
|---|---:|---|---|---|
| 001 | ~4 | late-night publishing office; establish Nari still away from home | A, B, N, WORK | none |
| 002 | ~4–5 | downstairs neighbor hears first dull human-weight movement | A, B | none; generic chapter-local neighbor interior crop |
| 003 | ~4 | more specific footsteps / `THUMP`; neighbor reacts | A, B | PREV required for neighbor appearance/interior crop continuity |
| 004 | ~3 | `SKRRR`; neighbor checks time; intercut confirms Nari still at work | A, B, N, WORK | PREV useful for neighbor continuity |
| 005 | ~3–4 | Nari finishes/leaves work after the noise time | A, B, N, WORK | PREV useful for office/Nari seam |
| 006 | ~4 | next morning; Nari encounters downstairs neighbor near corridor/elevator | A, B, N, APT | none; establish neighbor appearance here |
| 007 | ~5 | `Could you keep it down after midnight?` | A, B, N, APT | PREV required |
| 008 | ~5 | walking + chair scrape complaint specified | A, B, N, APT | PREV required |
| 009 | ~4 | Nari asks what time; neighbor answers | A, B, N, APT | PREV required |
| 010 | 2–3 | sparse hook: `I wasn't home.` | A, B, N, APT | PREV required |
| 011 | ~4 | normal publishing work resumes | A, B, N, WORK | none; scene reset |
| 012 | ~5–6 | optional unnamed coworker proof / dry humor | A, B, N, WORK | PREV required |
| 013 | ~4 | work/message/timestamp evidence confirms Nari was away | A, B, N, WORK | PREV useful |
| 014 | ~5 | Nari opens ordinary access history | A, B, N, WORK | PREV useful |
| 015 | ~3–4 | no ordinary entry; optional `...No entry.` | A, B, N, WORK | PREV useful |
| 016 | ~5 | mundane explanations: acoustics, wrong unit, app error, maintenance | A, B, N, WORK | PREV useful |
| 017 | ~5 | ordinary editorial work interrupts investigation; light comedy | A, B, N, WORK | PREV useful |
| 018 | ~3–4 | Nari decides to inspect home that evening | A, B, N, WORK | PREV useful |
| 019 | ~4 | ordinary return home; lock/door normal | A, B, N, APT | none; new location |
| 020 | ~5 | practical inspection: lock, windows, valuables, rooms | A, B, N, APT | PREV required |
| 021 | ~4 | nothing dramatic found; tension releases | A, B, N, APT | PREV required |
| 022 | ~3 | Nari notices Chair B slightly out of expected position | A, B, N, APT | PREV required |
| 023 | ~5 | self-doubt; optional `...Did I leave you like that?` | A, B, N, APT | PREV required |
| 024 | ~4 | Nari grips and aligns Chair B deliberately; restrained `SKK`/chair movement | A, B, N, APT | PREV required |
| 025 | ~3 | exact known Chair B position established; optional `There.` | A, B, N, APT | PREV required; this page becomes critical chair-state visual authority |
| 026 | ~5 | Nari returns to normal home routine | A, B, N, APT | PREV required |
| 027 | ~5–6 | meal/cleanup/domestic rhythm; chair unchanged | A, B, N, APT | PREV required |
| 028 | ~6 | optional offscreen phone/friend voice; comfortable ordinary conversation | A, B, N, APT | PREV required |
| 029 | ~5 | conversation shifts away from mystery | A, B, N, APT | PREV required |
| 030 | ~4 | late-night quiet; chair still exact | A, B, N, APT | PREV required |
| 031 | ~4–5 | bedtime preparation; ordinary `CLICK` etc. | A, B, N, APT | PREV required |
| 032 | ~3 | Nari leaves living/dining area; final verified empty-room state | A, B, N, APT | PREV required |
| 033 | ~2–3 | empty room; silence; Chair B unchanged | A, B, APT | PREV required |
| 034 | ~2 | extend stillness; no silhouette/body/shadow-person | A, B, APT | PREV required; chair must match Page 033/025 known state |
| 035 | ~2–3 | restrained `SKRRR`; movement source remains off-panel/unseen | A, B, APT | PREV required |
| 036 | 1–2 | final proof: Chair B displaced; no dialogue/narration | A, B, APT | PREV required; compare against known-state Page 025/034 |

## Page Dialogue / Script Authority

Each page-production file must copy final reader-visible wording from the approved Chapter 001 dialogue/story authorities and must not let the image model improvise wording.

Current important lines include:

- `Could you keep it down after midnight?`
- `Me?`
- `The chair. And the walking.`
- `What time?`
- `I wasn't home.`
- optional `...No entry.`
- optional `...Did I leave you like that?`
- optional `There.`
- final `SKRRR`

Final dialogue can be tightened during page compilation, but it may not reveal supernatural answers beyond the approved ceiling.

## Previous-Page WebP Rule

When a page marks `PREV required`, the page-production MD must list the exact previous approved page WebP path after that page exists.

Example for Page 024 after Page 023 is approved:

`manga/04-production/arc-01/chapter-001/page-023.webp`

Use PREV only as local seam authority for:

- pose/facing direction
- neighbor chapter-local identity
- room camera/prop state
- chair position
- ordinary clothing state
- ongoing action

The previous page may not override Nari canonical or the approved environment atlas.

## Page Production File Naming

Create exactly one Markdown generation authority per page:

`page-001-production.md`

through:

`page-036-production.md`

First generated review images:

`page-001.png` through `page-036.png`

Final approved repository images after manual conversion:

`page-001.webp` through `page-036.webp`

## Required Internal Sections Per Page

Every `page-###-production.md` must include:

1. Page Identity / Status
2. Source Authority
3. Exact Required Attachments / References
4. Page Canvas / Size Lock
5. Page Purpose / Scenario
6. Continuity Input
7. Page Composition / Panel Architecture
8. Complete Panel-by-Panel Blueprint
9. Exact Script / Lettering
10. Character Requirements
11. Environment / Object / Effect Requirements
12. Page-Wide Manga Style Lock
13. Complete Generation Instruction
14. Automatic Rejection Conditions
15. Continuity Output
16. QA / Approval Record

Follow:

`manga/04-production/page-production-standard.md`

## Automatic Chapter-Wide Rejection Conditions

Reject/regenerate any page that introduces:

- wrong Nari face/hair/body identity
- Nari copied from a generic Style A/B subject rather than her canonical WebP
- apartment geometry inconsistent with its master atlas
- workplace geometry/identity inconsistent with its master atlas
- wrong dining-chair count
- Chair B on the wrong designated side
- accidental Chair B movement before the scripted final movement
- visible Hyun-woo or any unexplained human body fragment from the hidden presence
- ghost/shadow-person imagery
- explicit blackout explanation
- invented supernatural effects
- premature horror intensity
- color, glossy webtoon finish, painterly grayscale, photorealism, CGI/3D, cinematic bloom/lighting
- unreadable right-to-left panel order
- invented dialogue, labels, logos, or fake text not required by the page script

## Generation Sequence

Generate sequentially:

Page 001 → review → approve → convert to WebP → Page 002 → continue.

Do not batch-generate the entire chapter blindly. The approved prior page becomes useful local continuity input for the next page when marked in the matrix.

## Final Chapter QA

Before Chapter 001 is declared complete, verify:

- all 36 page WebPs exist and correspond to their production MDs
- Nari identity remains stable across office/home scenes
- workplace and apartment geometry remain stable
- neighbor appears consistent across Pages 006–010 without becoming a falsely important mystery character
- Chair B exact known state is clearly established on Pages 024–025
- Chair B remains unchanged through Page 034 whenever visible
- no hidden person is visually revealed
- `SKRRR` on Page 035 is restrained and source-free
- Page 036 proves displacement without explanation
- Nari ends the chapter unaware of the final movement
- Chapter 002 can continue the unauthorized-activity/security mystery without contradiction
