# Refrigerator — Canonical Object + State Reference Prompt

Generate one production reference sheet for the SAME recurring refrigerator in Unit 2407. No characters.

BODY TURNAROUND:
- front, three-quarter, side views
- matte cool-gray body
- two upper doors
- lower freezer drawer
- recessed handles
- narrow upper-right integrated display
- no brand/logo
- nonreflective materials

STATE SEQUENCE — STATIC REFERENCE FRAMES THAT DEFINE EFFECT LANGUAGE:

A. NEUTRAL / DEAD DISPLAY
Narrow display fully dark.

B. HOSTILE WAKE FRAME
Corruption begins only at the narrow display: black pixels collect/crawl from display seams/edges; no full-door transformation.

C. HOSTILE DIAGNOSTIC TEXT
Broken/doubled stark-white glyphs type in segmented diagnostic progression. Representative sample: `TENANT PROFILE: PENDING`.

D. IDENTITY ACQUISITION FRAME
Representative sample: `YOON NARI` with `TIK... TIK...` typing feel contained to the display.

E. LOCKING FRAME
Representative sample: `IDENTITY LOCKING`, with slightly stronger seam-crawl/embedded corruption while remaining inside the narrow panel.

IMPORTANT:
- refrigerator hostile manifestation = embedded diagnostic/identity-lock intrusion
- do not use the TV's broad full-screen scanline/broadcast takeover language
- actual strip text always follows the current strip script; samples are effect references only
- reuse this exact refrigerator reference in every later chapter where the same Unit 2407 refrigerator returns
- no reflection, face/silhouette, room glow, full-door screen, photoreal/3D/gloss

Save local generation result as `refrigerator-canonical-states.png`.

After visual approval, manually convert and store/use this repository authority:
`refrigerator-canonical-reference.webp`
