# Strip 008 — Speaker Hidden / Paper Test / 1:17 A.M.

```text
Create exactly ONE tall full-color 2D manhwa TECHNICAL STRIP continuing Chapter 1 from APPROVED Strip 007.

PRODUCTION SCOPE
V36–V40. Historical P5 mapping only.

ATTACH — REQUIRED
- `Character-References/nari-canonical-flat2d.webp`
- `Manhwa/Environment-References/Unit-2407/unit-2407-floor-plan-reference.webp`
- `Manhwa/Environment-References/Unit-2407/unit-2407-fridge-cabinet-zone-detail-reference.webp`
- `Manhwa/Environment-References/Unit-2407/unit-2407-living-to-kitchen-wide-reference.webp`
- `Manhwa/Object-References/smart-speaker-canonical-reference.webp`
- APPROVED Strip 007
- this Strip 008 prompt

==================================================
BLACK READ-SLICE DIVIDER — ABSOLUTE
==================================================

Small black gutter between every distinct vertical beat. Side-by-side distinct shots use a slanted/diagonal black separator. Use a compact stronger black time divider before `1:17 A.M.`. Never let these separators become large empty bands.

==================================================
SPEAKER STORAGE ROUTE — ABSOLUTE
==================================================

At Strip 008 start:
- Nari is at/near entry after returning brass key
- canonical smart speaker is still UNPLUGGED on canonical kitchen counter
- lower speaker-storage cabinet is closed

V36 must show real movement:
1. Nari turns from entry and physically walks back to kitchen.
2. Reaches SAME unplugged speaker.
3. Opens SAME lower cabinet near refrigerator.
4. Uses SAME household storage stockpot, lid, and exactly two dish towels.
5. Places SAME unplugged screenless speaker into stockpot under towels.
6. Sets lid and closes cabinet.
SFX at storage action:
`CLUNK`

Do not merge storage stockpot with later noodle cooking pot.

==================================================
TWO-KNIFE IDENTITY LOCK — ABSOLUTE
==================================================

The guarded fruit knife introduced here is NOT the later practical blackout knife.
V38 uses a small household fruit knife with fitted guard. Nari secures the guard and hides that guarded fruit knife beneath the established sofa cushion.
SFX:
`SNICK`
The practical sheathed blackout knife does not exist in Unit 2407 yet; she buys it in Strip 011.

==================================================
VERTICAL STORY FLOW
==================================================

V36 — HIDE SPEAKER
Complete route/storage action above.

SMALL BLACK GUTTER.

V37 — PHYSICAL PAPER QUESTION
At established kitchen/toaster area, Nari places ONE ordinary paper note and SAME ordinary pen.
She handwrites exactly, wrapped as:
line 1: `WHO ARE`
line 2: `YOU?`
No digital font.

SMALL BLACK GUTTER.

V38 — GUARDED FRUIT KNIFE
Show guard being fitted, then Nari physically moves to sofa and slides guarded fruit knife beneath ONE sofa cushion.
SFX:
`SNICK`
No second fruit knife.

COMPACT BLACK TIME TRANSITION.

V39 — 1:17 A.M.
NARRATION:
`1:17 A.M.`
Nari is asleep/off-panel; no need to teleport her into scene.
Same hallway lamp gives one restrained flicker.
SFX at hallway light:
`FZZT`

SMALL BLACK GUTTER / SUSPENSE.

V40 — PEN MOVES
Same pen physically moves across SAME note paper.
SFX:
`SCRATCH... SCRATCH...`
No hand, body, shadow-person, glowing writing, or new device.

==================================================
END STATE
==================================================

- brass key = entry tray
- binder = kitchen counter
- speaker = hidden unplugged inside storage stockpot / lower cabinet CLOSED
- guarded fruit knife = beneath sofa cushion
- note + pen = same kitchen/toaster area

NO DEAD BOTTOM — ABSOLUTE
End on pen/paper/night atmosphere with compact black story gutter only. No giant blank tail.

SEAM OUT — G / TIME CUT
Strip 009 begins next morning with SAME paper/pen design.

AUTOMATIC REJECT IF
- speaker teleports to cabinet
- speaker is plugged in
- storage pot merges with noodle pot
- fruit knife is confused with practical blackout knife
- note/pen duplicates
- visible hand/body writes the reply
- black separators missing/oversized
- giant dead bottom appears
```

## Production Thread Attachments
1. `Character-References/nari-canonical-flat2d.webp`
2. `Manhwa/Environment-References/Unit-2407/unit-2407-floor-plan-reference.webp`
3. `Manhwa/Environment-References/Unit-2407/unit-2407-fridge-cabinet-zone-detail-reference.webp`
4. `Manhwa/Environment-References/Unit-2407/unit-2407-living-to-kitchen-wide-reference.webp`
5. `Manhwa/Object-References/smart-speaker-canonical-reference.webp`
6. APPROVED Strip 007
7. this Strip 008 prompt
