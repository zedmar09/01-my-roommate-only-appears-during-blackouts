# Strip 011 — Blackout Kit / Apartment Preparation

```text
Create exactly ONE tall full-color 2D manhwa TECHNICAL STRIP continuing Chapter 1.

PRODUCTION SCOPE
V51–V54. Historical P7 mapping only.

ATTACH — REQUIRED
- Nari canonical
- `Manhwa/Environment-References/Unit-2407/unit-2407-floor-plan-canonical.png`
- `Manhwa/Environment-References/Unit-2407/unit-2407-fridge-cabinet-zone-detail-canonical.png`
- `Manhwa/Environment-References/Unit-2407/unit-2407-desk-zone-detail-canonical.png`
- APPROVED Strip 010
- this Strip 011 prompt

==================================================
BLACK READ-SLICE DIVIDER — ABSOLUTE
==================================================

Every distinct vertical slice gets a SMALL BLACK GUTTER. Separate side-by-side acquisition/detail views with a DIAGONAL BLACK DIVIDER. Use a compact black location transition between supply store and return to Unit 2407. No giant dead space.

==================================================
V51–V52 SUPPLY STORE / PURCHASE COUNT
==================================================

V51
NARRATION:
`LATER THAT DAY`
Neighborhood home-supply mart/supermarket; ordinary, no brand/logo clutter.

SMALL BLACK GUTTER.

V52
NARRATION:
`SHE STARTED WITH A BLACKOUT KIT.`
Show a flowing acquisition/checkout montage containing the intended categories:
- candles
- ceramic holders
- matches
- batteries
- battery flashlight
- portable radio
- manual can opener
- ONE practical knife already inside its protective sheath

This practical sheathed knife is a DIFFERENT object from guarded fruit knife already hidden under sofa cushion.
Do not duplicate either knife.

COMPACT BLACK LOCATION TRANSITION.

==================================================
V53 RETURN HOME
==================================================

Nari returns carrying exactly TWO ordinary shopping bags.
She physically unlocks/crosses SAME Unit 2407 threshold.
SFX at smart lock:
`BEEP`
No teleport directly from store to pantry.

SMALL BLACK GUTTER.

==================================================
V54 PREPARATION / EXACT COUNTS
==================================================

Use a tall preparation run with canonical pantry + desk zones.

PANTRY:
- supplies stored logically
- exactly SIX emergency noodle packets total at this time
- one spicy-seafood flavor + five other flavors
- no extra seventh packet

NARRATION:
`FOOD, CANDLES, MATCHES, AND SPARES WENT IN THE PANTRY.`

DESK:
Emergency tray immediately RIGHT of keyboard receives exactly:
- battery flashlight
- portable radio
- SAME practical knife fully sheathed, handle toward Nari's chair

NARRATION:
`THE FLASHLIGHT, RADIO, AND KNIFE STAYED WITHIN REACH.`

Hallway mains lamp visibly ON.
Guarded fruit knife remains under sofa cushion and is not shown as part of desk tray.
If pantry and desk share one horizontal row, separate them with a strong slanted black divider so they read as different zones.

==================================================
END STATE / REAL-SCENARIO CHECK
==================================================

- six noodles in pantry
- practical knife sheathed at desk tray
- fruit knife guarded under sofa
- binder kitchen counter
- brass key entry tray
- speaker hidden unplugged inside closed lower cabinet
- hallway lamp on
- apartment normally powered

NO DEAD BOTTOM — ABSOLUTE
End on organized canonical apartment / desk-preparedness composition, not giant blank tail.

SEAM OUT — G / TIME CUT
Strip 012 begins two days later after work at building entrance.

AUTOMATIC REJECT IF
- practical knife/fruit knife merge
- noodle count is not exactly six
- emergency tray moves from right of keyboard
- Nari has more/fewer than two shopping bags on return
- apartment geography drifts
- black separators missing/oversized
- giant dead bottom appears
```

## Production Thread Attachments
1. Nari canonical
2. Unit 2407 floor plan
3. Unit 2407 fridge/cabinet detail
4. Unit 2407 desk-zone detail
5. APPROVED Strip 010
6. this Strip 011 prompt
